  ...
  if (cmd) {
    ...
    if (cd) {
      ...
      if (diff < cooldown)
        if (!sended) {
          sended = true;
          return message.channel.send(embed.setDescription(`Bu komutu tekrar kullanabilmek için **${Number(((cooldown - diff) / 1000).toFixed(2))}** saniye daha beklemelisin!`)).then((x) => x.delete({ timeout: (cooldown - diff) }));
        }
