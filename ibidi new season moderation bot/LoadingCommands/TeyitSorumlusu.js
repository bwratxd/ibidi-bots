module.exports.operate = async ({client, msg, author, member, auth}, Discord = require("discord.js")) => {
    if ((!author.roles.cache.some(r => auth.Ability.TeyitLideri.includes(r.id))) && (!author.permissions.has("ADMINISTRATOR"))) return;
  if (!member) return client.message(client.noMember(msg), msg.channel.id, 6500);
      await member.roles.add(auth.Ability.TeyitSorumlusu);
    msg.react(client.react("duztik"));
     client.message({embed: { 
      ...
      description: `${author} yetkili tarafından ${member} adlı üyeye başarıyla <@&${auth.Ability.TeyitSorumlusu}> rolü verilmiştir.`,
