module.exports.operate = async ({client, msg, author, member, auth}, Discord = require("discord.js")) => {
     if ((!author.roles.cache.some(r => auth.Ability.ChatLideri.includes(r.id))) && (!author.permissions.has("ADMINISTRATOR"))) return;
     if (!member) return client.message(client.noMember(msg), msg.channel.id, 6500);
     await member.roles.add(auth.Ability.ChatSorumlusu);
     ...
     client.message({embed: { 
      ...
      description: `${author} yetkili tarafından ${member} adlı üyeye başarıyla <@&${auth.Ability.ChatSorumlusu}> rolü verilmiştir.`,
